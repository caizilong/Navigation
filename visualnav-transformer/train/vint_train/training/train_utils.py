...
def _compute_losses(
    ...
    action_loss = action_reduce(F.mse_loss(
...
def train(
    ...
    action_loss_logger = Logger(
        "action_loss", "train", window_size=print_log_freq)
    ...
    total_loss_logger = Logger(
        "total_loss", "train", window_size=print_log_freq)
    loggers = {
    ...
    for i, data in enumerate(tqdm_iter):
        ...
        obs_images = [transform(obs_image).to(device)
                      for obs_image in obs_images]
...
def train_nomad(
    ...
    gc_dist_loss_logger = Logger(
        "gc_dist_loss", "train", window_size=print_log_freq)
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            obsgoal_cond = model("vision_encoder", obs_img=batch_obs_images,
                                 goal_img=batch_goal_images, input_goal_mask=goal_mask)
            ...
            noise_pred = model("noise_pred_net", sample=noisy_action,
                               timestep=timesteps, global_cond=obsgoal_cond)
...
def evaluate_nomad(
    ...
    gc_dist_loss_logger = Logger(
        "gc_dist_loss", eval_type, window_size=print_log_freq)
    gc_action_loss_logger = Logger(
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            ### RANDOM MASK ERROR ###
            # Predict the noise residual
            rand_mask_noise_pred = ema_eval_model(
                "noise_pred_net", sample=noisy_actions, timestep=timesteps, global_cond=rand_mask_cond)

            # L2 loss
            rand_mask_loss = nn.functional.mse_loss(
                rand_mask_noise_pred, noise)

            ### NO MASK ERROR ###
            # Predict the noise residual
            no_mask_noise_pred = ema_eval_model(
            ...
            ### GOAL MASK ERROR ###
            # predict the noise residual
            goal_mask_noise_pred = ema_eval_model(
                "noise_pred_net", sample=noisy_actions, timestep=timesteps, global_cond=goal_mask_cond)

            # L2 loss
            goal_mask_loss = nn.functional.mse_loss(
                goal_mask_noise_pred, noise)
...
def model_output(
    ...
    obsgoal_cond = model("vision_encoder", obs_img=batch_obs_images,
                         goal_img=batch_goal_images, input_goal_mask=no_mask)
    # obsgoal_cond = obsgoal_cond.flatten(start_dim=1)
    ...
    for k in noise_scheduler.timesteps[:]:
        ...
        noise_pred = model(
            ...
            timestep=k.unsqueeze(-1).repeat(
                diffusion_output.shape[0]).to(device),
...
def visualize_diffusion_action_distribution(
    ...
    num_images_log = min(
        num_images_log, batch_obs_images.shape[0], batch_goal_images.shape[0], batch_action_label.shape[0], batch_goal_pos.shape[0])
    ...
    for i in range(num_images_log):
        ...
        traj_colors = ["red"] * len(uc_actions) + \
