...
class MambaViNT(BaseModel):
    ...
    def forward(
        ...
        if len(goal_encoding.shape) == 2:
            goal_encoding = goal_encoding.unsqueeze(1)  # [batch, 1, encoding_size]
        ...
        obs_encoding = torch.transpose(obs_encoding, 0, 1)  # [batch, context+1, encoding_size]
        
        # 3. 拼接观测和目标 - 形成完整序列
        # 参考MTIL: 序列长度 = context_size+2 (历史帧 + 当前帧 + 目标)
        tokens = torch.cat((obs_encoding, goal_encoding), dim=1)  # [batch, context+2, encoding_size]
    ...
    def step(
        ...
        for i, blk in enumerate(self.mamba_blocks):
            ...
            if hasattr(blk.mixer, "step"):
                ...
                y_t = y_t.squeeze(1)  # [batch, 1, encoding_size] -> [batch, encoding_size]
            else:
                # 降级处理：无step方法时
